/**
 * Custom Cache Strategy for handling BO data
 * @param {type} persistenceManager
 * @param {type} persistenceUtils
 * @param {type} logger
 * @return {undefined}
 */
define(['persist/persistenceManager',
    'persist/persistenceUtils', 'persist/impl/logger'
], function (persistenceManager, persistenceUtils, logger) {
    'use strict';
    ...
    function getHttpCacheHeaderStrategy() {
        return function (request, response) {
            ...
            return _handleIfCondMatch(request, response).then(function (
                response) {
    ...
    function _handleExpires(request, response) {
        ...
        var cacheExpirationDate = response.headers.get(
    ...
    function _handleMaxAge(request, response) {
        ...
        var cacheControlMaxAge = _getCacheControlDirective(response.headers,
            'max-age');
        ...
        if (ifMatch || ifNoneMatch) {
            if (!persistenceManager.isOnline()) {
                ...
                if (ifMatch &&
                    ...
                    // spec
                    return persistenceUtils.responseToJSON(response).then(function (
                        responseData) {
                        responseData.status = 412;
                        responseData.statusText =
                            'If-Match failed due to no matching ETag while offline';
                        logger.log(
                    ...
                    return persistenceUtils.responseToJSON(response).then(function (
                        ...
                        responseData.status = 412;
                        responseData.statusText =
                            'If-None-Match failed due to matching ETag while offline';
                        logger.log(
    ...
    function _handleMustRevalidate(request, response) {
        ...
        var mustRevalidate = _getCacheControlDirective(response.headers,
            'must-revalidate');
        if (mustRevalidate) {
            var cacheExpirationDate = response.headers.get(
                'x-oracle-jscpt-cache-expiration-date');
            if (cacheExpirationDate) {
                ...
                if (currentTime > cacheExpirationTime) {
                    logger.log(
                        "Offline Persistence Toolkit cacheStrategies: Handling revalidation HTTP must-revalidate header"
                    );
    ...
    function _isNoCache(request, response) {
        if (_getCacheControlDirective(response.headers, 'no-cache')) {
            logger.log(
                "Offline Persistence Toolkit cacheStrategies: Has HTTP no-cache header"
            );
            return true;
        }
        // pragma: no-cache in the request header has the same effect
        var pragmaNoCache = request.headers.get('Pragma');
        var isPragmeNoCache = pragmaNoCache && (pragmaNoCache.trim() ===
            'no-cache');

        if (isPragmeNoCache) {
            logger.log(
                "Offline Persistence Toolkit cacheStrategies: Has HTTP Pragma no-cache header"
            );
        }
            ...
            if (!persistenceManager.isOnline()) {
                ...
                if (mustRevalidate) {
                    return persistenceUtils.responseToJSON(response).then(function (
                        ...
                        responseData.statusText =
                            'cache-control: must-revalidate failed due to application being offline';
                        logger.log(
                ...
                return persistenceManager.browserFetch(request).then(function (
                    serverResponse) {
    ...
    function _cacheResponse(request, response) {
        ...
        if (response != null &&
            ...
            return persistenceManager.getCache().put(request, response).then(
                function () {
                    logger.log(
                        "Offline Persistence Toolkit cacheStrategies: Cached Request/Response"
                    );
